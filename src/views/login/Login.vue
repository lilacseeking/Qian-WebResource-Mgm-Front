  ...
  <div class="container">
    ...
    <div class="main">
      <el-tabs class="custom-tab" v-model="activeName">
        <el-tab-pane disabled label="" name="0"></el-tab-pane>
        <el-tab-pane disabled label="" name="0"></el-tab-pane>
        ...
        <el-form class="custom-form" label-position="left" label-width="70px" :model="loginUser" ref="usernameForm" :rules="rules">
          ...
          <el-form-item>
            <el-button type="primary" :loading="usernameLoginLoading" @click="loginByUsername('usernameForm')" :disabled="!loginUser.username || !loginUser.password" style="width:100%;">登录</el-button>
            <!-- <el-button @click="handleReset">重置</el-button> -->
          </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      envDefaultClick() {
        this.loginUser.username = 'dev_system';
        let access_token = this.rundomStr(32);
        sessionStorage.setItem('user', '{"username":"' + this.loginUser.username + '","token":"' + access_token + '"}');
        this.setCookie(this.loginUser.username, access_token);
        this.$router.push({ path: '/' });
        this.$message({
          showClose: true,
          message: '您已经登录成功',
      ...
      autoLog() {
        if (this.autoLogin) {
          ...
          if (user) {
            ...
            sessionStorage.setItem('user', '{"username":"' + userObj.username + '","token":"' + userObj.token + '"}');
            this.$router.push({ path: '/' });
            this.$message({
              showClose: true,
              message: '自动登录成功',
      ...
      setCookie(username, token) {
        if (this.autoLogin) {
          this.$cookieStore.setCookie('user', JSON.stringify({username: username, token: token}), 60*60*12);
        }
      }
    },
    mounted() {
